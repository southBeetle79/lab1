...
int main()
{
   ...
   //создается неявный конструктор, который определяет компилятор, он ничего не делает, так как поля остаются неинициализированными
   //при выходе вызывается неявный деструктор, который определяется компилятором, и просто помечает память на стеке как свободную
   ...
   //так как мы объявили конструктор с 4 параметрами, компилятор перестал автоматически создавать неявный конструктор по умолчанию
    ...
    {
        Rect rect1(1, 2, 3, 4);//явный конструктор с параметрами, который инициализирует поля конкретными числами
        Rect rect2 = rect1;//неявный конструктор копирования, вызывается конструктор, определил компилятор
        Rect rect3(rect1);//неявный конструктор копирования, вызывается конструктор, определил компилятор
    }
     ...
     */
    //деструктор это метод класса, который завершает жизненный цикл объекта, вызывается автоматически, в классе Rect есть деструктор
    //хотя я его не писал, но компилятор автоматически создал неявный деструкто, в моем случае деструктор просто позволяет памяти,
    //которую занимал объект на стеке, быть переиспользованной другими частями программы, а в общем случае деструктор играет
    //важнейшую роль для освобождения ресурсов.
...

    {
        ...
        Rect *pR = new Rect(1,2,1,2);//конструктор с параметрами(объект в куче)
        {
            Rect r2(r1);//конструктор по умолчанию
            Rect arRect[2];//конструктор по умолчанию
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ;//конструктор с параметрами(только при i = 0)
                Rect r4(*pR);//конструктор копирования(каждую итерацию)
   ...
   //private подразумевает защиту данных от изменения, соответственно взаимодействовать напрямую с такими данными нельзя
     ...
     */
     ...
     //функция член - это поведение, неразрывно связанное с данными объекта, а обычная функция это просто изолированный блок кода
    ...
    {
        Rect r1(1, 2, 3, 4), r2(20, 25, 30, 35);
...

    {
        ...
        cout << "Width = " << r.get_width() << ", Height = " << r.get_height() << endl;
        cout << "Square = " << r.get_square() << endl;
        
        r.set_width(20);
        r.set_height(4);
        cout << "New square = " << r.get_square() << endl;
        cout << "New right = " << r.get_right() << endl;
    }
